...
def init_set():
    print(u"請登入Holodex網站(https://holodex.net/)，")
    print(u"點選右上角帳號圖標，並在彈出選單中選擇帳號設定")
    print(u"拉到該頁面的最下方，取得API金鑰後將他貼在下方\n")
    print(u"您也可以直接按enter鍵跳過這個步驟，")
    print(u"但是未設定金鑰將有機率被Holdex拒絕服務而導致程式錯誤\n")
    while True:
        apikey=input(u"請輸入您的Holodex API金鑰：")
        if apikey == "":
            print(u"未設定金鑰，返回主選單\n")
            time.sleep(1)
            return
            ...
            print(u"金鑰格式不符，請重新設定\n")

def set_mode():
    while True:
        print(u"使用模式列表:")
        print(u"(1).下載單支影片或直播")
        print(u"(2).紀錄特定頻道未來所有直播與影片")
        print(u"(3).紀錄特定頻道特定標題直播與影片")
        print(u"(s)et.設定Holodex API金鑰")
        print(u"(e)xit.離開本服務\n")
        mode = input(u"請選擇使用模式(1/2/3/s/e)：")
        if mode == "1":
            print(u"使用模式1：下載單支影片或直播")
            return mode
        elif mode == "2":
            print(u"使用模式2：紀錄特定頻道未來所有直播與影片")
            return mode
        elif mode == "3":
            print(u"使用模式3：紀錄特定頻道特定標題直播與影片")
            return mode
        elif mode == "set" or mode == "s":
            print(u"設定Holodex API金鑰")
            return "set"
        elif mode == "exit" or mode == "e":
            print(u"感謝您使用本服務")
            return "exit"
        else:
            print(u"請輸入數字1或2或3或字母s或e\n")
            time.sleep(1)

def set_live_id():
    while True:
        print(u"\n\n請輸入影片或直播網址或ID\n例如：\nhttps://www.youtube.com/watch?v=9yBLZKFKXyg 或是")
        print(u"https://youtu.be/9yBLZKFKXyg 或是")
        print(u"9yBLZKFKXyg\n")
        id = input(u"請輸入影片或直播網址或ID：")
        if 'youtu.be' in id:
            id = id.split('/')[-1]
        elif 'youtube' in id:
            id = id.split('v=')[-1]
        ...
        try:
            ...
            if live.get('title', 0) != 0:
                print(u"您要記錄的影片或直播為"+live.get('title', 0))
                return id
            else:
                print(u"\n無法找到影片或直播，可能是網址格式錯誤或不支援的影片或直播\n")
                time.sleep(1)
...
def set_channel_id():
    while True:
        ...
        print(u"UC-hM6YJuNYVAmUWxeIr9FeA\n")
        id = input(u"請輸入頻道網址或ID：")
        if  'youtube' in id:
            id = id.split('/')[-1]
        elif id == "":
            print(u"\n請輸入頻道id\n")
        ...
        if channel.get('name', 0) != 0:
            print(u"您要記錄的頻道為"+channel.get('name', 0))
            return id
        else:
...
def set_keyword():
    while True:
        keyword = input(u"請輸入要記錄的影片或直播關鍵字(不分大小寫)：").lower()
        if keyword == "":
            return None
        return keyword
...
def main():
    print(u"vtuber紀錄小幫手 0.1.3-alpha")
    print(u"該服務基於Holodex API與yt-dlp所開發，目前僅支援Holodex所收錄的vtuber與剪輯\n\n")
    config = configparser.ConfigParser()
    config.read('config.ini')
